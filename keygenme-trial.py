## PicoCTF Chall keygenme-trial.py
...
##-----------------------------------------------SOLVE------------------------------------------------------------------------------##
# I referenced the funcion check_key() on line 156 to create the hash variable. 

    hash = hashlib.sha256(b"FREEMAN").hexdigest()

# here I print out each char in the hashed var based on the postion in the check_key() function

    print(key_part_static1_trial + hash[4] + hash[5] + hash[3] + hash[6] + hash[2] + hash[7] + hash[1] + hash[8] + "}")
